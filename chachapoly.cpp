...
{
	CryptoPP_NV_2_ASSERT(userKey && userKeyLength == 32);
...
{
	CryptoPP_NV_2_ASSERT(iv && len == 12);
...
{
	CryptoPP_NV_2_ALIGN_DATA(8) byte length[2*sizeof(word64)];
...
{
	CryptoPP_NV_2_ASSERT(iv && len == 24);
	RekeyCipherAndMac(m_userKey, m_userKey.SizeInBytes(),
		MakeParameters(Name::IV(), ConstByteArrayParameter(iv,len)));
}
