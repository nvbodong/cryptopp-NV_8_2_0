...
#   http://github.com/weidai11/CryptoPP_NV_2/issues/190
...
DETECT_FEATURES ?= 1
ifeq ($(findstring -DCryptoPP_NV_2_DISABLE_ASM,$(CXXFLAGS)),-DCryptoPP_NV_2_DISABLE_ASM)
  DETECT_FEATURES := 0
...
ifeq ($(DETECT_FEATURES),1)
  ...
  ifeq ($(SSE2_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_ASM
  else ifeq ($(SSE3_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_SSE3
  else ifeq ($(SSSE3_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_SSSE3
  else ifeq ($(SSE41_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_SSE4
  ...
  ifneq ($(SSE42_FLAG),)
    ...
    ifeq ($(CLMUL_FLAG),)
      CXXFLAGS += -DCryptoPP_NV_2_DISABLE_CLMUL
    endif
    ifeq ($(AESNI_FLAG),)
      CXXFLAGS += -DCryptoPP_NV_2_DISABLE_AESNI
    endif

    ifeq ($(AVX_FLAG),)
      CXXFLAGS += -DCryptoPP_NV_2_DISABLE_AVX
    else ifeq ($(AVX2_FLAG),)
      CXXFLAGS += -DCryptoPP_NV_2_DISABLE_AVX2
    else ifeq ($(SHANI_FLAG),)
      CXXFLAGS += -DCryptoPP_NV_2_DISABLE_SHANI
    endif
  endif
...
ifeq ($(findstring -DCryptoPP_NV_2_DISABLE_ASM,$(CXXFLAGS)),)
...
ifneq ($(strip $(HAVE_OPT)),0)
  CXXFLAGS += -DCryptoPP_NV_2_DISABLE_MIXED_ASM
endif
...
ifeq ($(IS_ARMV8),1)
  ...
  HAVE_OPT = $(shell $(CXX) $(TCXXFLAGS) $(ZOPT) $(TOPT) $(TPROG) -o $(TOUT) 2>&1 | tr ' ' '\n' | wc -l)
  ifeq ($(strip $(HAVE_OPT)),0)
	ACLE_FLAG += -DCryptoPP_NV_2_ARM_ACLE_AVAILABLE=1
  else
	CXXFLAGS += -DCryptoPP_NV_2_ARM_ACLE_AVAILABLE=0
  endif
  ...
  ifneq ($(ASIMD_FLAG),)
    ...
    ifeq ($(strip $(HAVE_OPT)),0)
      ...
      CXXFLAGS += -DCryptoPP_NV_2_ARM_CRC32_AVAILABLE=0
    endif
    ...
    ifeq ($(strip $(HAVE_OPT)),0)
      ...
      CXXFLAGS += -DCryptoPP_NV_2_ARM_AES_AVAILABLE=0
    endif
    ...
    ifeq ($(strip $(HAVE_OPT)),0)
      ...
      CXXFLAGS += -DCryptoPP_NV_2_ARM_PMULL_AVAILABLE=0
    endif
    ...
    ifeq ($(strip $(HAVE_OPT)),0)
      ...
      CXXFLAGS += -DCryptoPP_NV_2_ARM_SHA_AVAILABLE=0
    endif
...
ifeq ($(DETECT_FEATURES),1)
  ...
  ifeq ($(ALTIVEC_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_ALTIVEC
  else ifeq ($(POWER8_FLAG)$(POWER7_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_POWER7
  else ifeq ($(POWER8_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_POWER8
  else ifeq ($(POWER9_FLAG),)
    CXXFLAGS += -DCryptoPP_NV_2_DISABLE_POWER9
  endif
...
ifeq ($(IS_SUN)$(SUN_COMPILER),11)
  ifneq ($(IS_X86)$(IS_X64),00)
    ifeq ($(findstring -DCryptoPP_NV_2_DISABLE_ASM,$(CXXFLAGS)),)
      LDFLAGS += -M CryptoPP_NV_2.mapfile
    endif  # No CryptoPP_NV_2_DISABLE_ASM
  endif  # X86/X32/X64
endif  # SunOS
...
# -DCryptoPP_NV_2_INCLUDE_VECTOR_CC is needed for Sun Studio 12u1 Sun C++ 5.10 SunOS_i386 128229-02 2009/09/21
# and was fixed in May 2010. Remove it if you get "already had a body defined" errors in vector.cc
CXXFLAGS += -DCryptoPP_NV_2_INCLUDE_VECTOR_CC
endif
AR = $(CXX)
ARFLAGS = -xar -o
RANLIB = true
...
ifeq ($(findstring ubsan,$(MAKECMDGOALS)),ubsan)
  ...
  ifeq ($(findstring -DCryptoPP_NV_2_COVERAGE,$(CXXFLAGS)),)
    CXXFLAGS += -DCryptoPP_NV_2_COVERAGE
  endif # CXXFLAGS
endif # UBsan
...
ifneq ($(filter valgrind,$(MAKECMDGOALS)),)
  ...
  CXXFLAGS := $(CXXFLAGS:-O%=-O1)
  CXXFLAGS := $(CXXFLAGS:-xO%=-xO1)
  ifeq ($(findstring -DCryptoPP_NV_2_VALGRIND,$(CXXFLAGS)),)
    CXXFLAGS += -DCryptoPP_NV_2_VALGRIND
  endif # -DCryptoPP_NV_2_VALGRIND
endif # Valgrind
...
LIB_VER := $(shell $(GREP) "define CryptoPP_NV_2_VERSION" config.h | cut -d" " -f 3)
...
SOLIB_FLAGS=-Wl,-soname,libCryptoPP_NV_2.so$(SOLIB_COMPAT_SUFFIX)
endif
# Solaris uses -Wl,-h
ifeq ($(IS_SUN),1)
...
SOLIB_FLAGS=-Wl,-h,libCryptoPP_NV_2.so$(SOLIB_COMPAT_SUFFIX)
endif
...
# https://github.com/weidai11/CryptoPP_NV_2/issues/738
...
# The 5.6.2 list is at https://github.com/weidai11/CryptoPP_NV_2/blob/789f81f048c9.
DLLSRCS := $(SRCS)
...
ifneq ($(IS_DARWIN),0)
static: libCryptoPP_NV_2.a
shared dynamic dylib: libCryptoPP_NV_2.dylib
else
static: libCryptoPP_NV_2.a
shared dynamic: libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX)
endif

.PHONY: dep deps depend
dep deps depend GNUmakefile.deps:
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_DISABLE_ASM) -MM *.cpp > GNUmakefile.deps
...
valgrind: cryptest.exe
	valgrind --track-origins=yes --suppressions=CryptoPP_NV_2.supp ./cryptest.exe v
...
docs html:
	...
	@-$(RM) CryptoPP_NV_2Ref.zip
	doxygen Doxyfile -d CryptoPP_NV_2_DOXYGEN_PROCESSING
	$(MV) $(DOXYGEN_DIRECTORY)/ $(DOCUMENT_DIRECTORY)/
	zip -9 CryptoPP_NV_2Ref.zip -x ".*" -x "*/.*" -r $(DOCUMENT_DIRECTORY)/

.PHONY: clean
clean:
	-$(RM) adhoc.cpp.o adhoc.cpp.proto.o $(LIBOBJS) rdrand-*.o $(TESTOBJS) $(DLLOBJS) $(LIBIMPORTOBJS) $(TESTIMPORTOBJS) $(DLLTESTOBJS)
	@-$(RM) libCryptoPP_NV_2.a libCryptoPP_NV_2.dylib CryptoPP_NV_2.dll libCryptoPP_NV_2.dll.a libCryptoPP_NV_2.import.a
	@-$(RM) libCryptoPP_NV_2.so libCryptoPP_NV_2.so$(SOLIB_COMPAT_SUFFIX) libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX)
	@-$(RM) cryptest.exe dlltest.exe cryptest.import.exe cryptest.info ct et
	@-$(RM) *.la *.lo *.gcov *.gcno *.gcda *.stackdump core core-*
	@-$(RM) /tmp/adhoc.exe
	@-$(RM) -r /tmp/CryptoPP_NV_2_test/
...
autotools-clean:
	...
	@-$(RM) -f cryptest cryptestcwd libtool* libCryptoPP_NV_2.la libCryptoPP_NV_2.pc*
	@-$(RM) -rf m4/ auto*.cache/ .deps/ .libs/

.PHONY: cmake-clean
cmake-clean:
	@-$(RM) -f CryptoPP_NV_2-config.cmake CMakeLists.txt
	@-$(RM) -rf cmake_build/

.PHONY: distclean
distclean: clean autotools-clean cmake-clean
	-$(RM) adhoc.cpp adhoc.cpp.copied GNUmakefile.deps benchmarks.html cryptest.txt
	@-$(RM) cryptest-*.txt CryptoPP_NV_2.tgz libCryptoPP_NV_2.pc *.o *.bc *.ii *~
	...
	@-$(RM) CryptoPP_NV_2$(LIB_VER)\.* CryptoPP_NV_2Ref.zip

# Install cryptest.exe, libCryptoPP_NV_2.a, libCryptoPP_NV_2.so and libCryptoPP_NV_2.pc.
...
install: cryptest.exe install-lib
	...
	@-$(MKDIR) $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestData
	@-$(MKDIR) $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestVectors
	$(CP) TestData/*.dat $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestData
	$(CHMOD) 0644 $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestData/*.dat
	$(CP) TestVectors/*.txt $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestVectors
	$(CHMOD) 0644 $(DESTDIR)$(DATADIR)/CryptoPP_NV_2/TestVectors/*.txt

# A recipe to install only the library, and not cryptest.exe. Also
# see https://github.com/weidai11/CryptoPP_NV_2/issues/653. Some users
# already have a libCryptoPP_NV_2.pc. Install the *.pc file if the file
# is present. If you want one, then issue 'make libCryptoPP_NV_2.pc'.
.PHONY: install-lib
install-lib:
	@-$(MKDIR) $(DESTDIR)$(INCLUDEDIR)/CryptoPP_NV_2
	$(CP) *.h $(DESTDIR)$(INCLUDEDIR)/CryptoPP_NV_2
	$(CHMOD) 0644 $(DESTDIR)$(INCLUDEDIR)/CryptoPP_NV_2/*.h
ifneq ($(wildcard libCryptoPP_NV_2.a),)
	@-$(MKDIR) $(DESTDIR)$(LIBDIR)
	$(CP) libCryptoPP_NV_2.a $(DESTDIR)$(LIBDIR)
	$(CHMOD) 0644 $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.a
endif
ifneq ($(wildcard libCryptoPP_NV_2.dylib),)
	@-$(MKDIR) $(DESTDIR)$(LIBDIR)
	$(CP) libCryptoPP_NV_2.dylib $(DESTDIR)$(LIBDIR)
	$(CHMOD) 0755 $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.dylib
	-install_name_tool -id $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.dylib $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.dylib
endif
ifneq ($(wildcard libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX)),)
	@-$(MKDIR) $(DESTDIR)$(LIBDIR)
	$(CP) libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX) $(DESTDIR)$(LIBDIR)
	$(CHMOD) 0755 $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX)
ifeq ($(HAS_SOLIB_VERSION),1)
	-$(LN) libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.so
	$(LDCONF) $(DESTDIR)$(LIBDIR)
endif
endif
ifneq ($(wildcard libCryptoPP_NV_2.pc),)
	@-$(MKDIR) $(DESTDIR)$(LIBDIR)/pkgconfig
	$(CP) libCryptoPP_NV_2.pc $(DESTDIR)$(LIBDIR)/pkgconfig
	$(CHMOD) 0644 $(DESTDIR)$(LIBDIR)/pkgconfig/libCryptoPP_NV_2.pc
endif

.PHONY: remove uninstall
remove uninstall:
	-$(RM) -r $(DESTDIR)$(INCLUDEDIR)/CryptoPP_NV_2
	-$(RM) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.a
	-$(RM) $(DESTDIR)$(BINDIR)/cryptest.exe
	@-$(RM) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.dylib
	@-$(RM) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX)
	@-$(RM) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.so$(SOLIB_COMPAT_SUFFIX)
	@-$(RM) $(DESTDIR)$(LIBDIR)/libCryptoPP_NV_2.so
	@-$(RM) $(DESTDIR)$(LIBDIR)/pkgconfig/libCryptoPP_NV_2.pc
	@-$(RM) -r $(DESTDIR)$(DATADIR)/CryptoPP_NV_2

libCryptoPP_NV_2.a: $(LIBOBJS)
	$(AR) $(ARFLAGS) $@ $(LIBOBJS)
ifeq ($(IS_SUN),0)
	$(RANLIB) $@
endif
...
ifeq ($(HAS_SOLIB_VERSION),1)
.PHONY: libCryptoPP_NV_2.so
libCryptoPP_NV_2.so: libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX) | so_warning
endif

libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX): $(LIBOBJS)
ifeq ($(XLC_COMPILER),1)
...
ifeq ($(HAS_SOLIB_VERSION),1)
	-$(LN) libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX) libCryptoPP_NV_2.so
	-$(LN) libCryptoPP_NV_2.so$(SOLIB_VERSION_SUFFIX) libCryptoPP_NV_2.so$(SOLIB_COMPAT_SUFFIX)
endif

libCryptoPP_NV_2.dylib: $(LIBOBJS)
...
cryptest.exe:libCryptoPP_NV_2.a $(TESTOBJS)
	$(CXX) -o $@ $(strip $(CXXFLAGS)) $(TESTOBJS) ./libCryptoPP_NV_2.a $(LDFLAGS) $(LDLIBS)
...
CryptoPP_NV_2.dll: $(DLLOBJS)
	$(CXX) -shared -o $@ $(strip $(CXXFLAGS)) $(DLLOBJS) $(LDFLAGS) $(LDLIBS) -Wl,--out-implib=libCryptoPP_NV_2.dll.a

libCryptoPP_NV_2.import.a: $(LIBIMPORTOBJS)
	$(AR) $(ARFLAGS) $@ $(LIBIMPORTOBJS)
ifeq ($(IS_SUN),0)
	$(RANLIB) $@
endif

cryptest.import.exe: CryptoPP_NV_2.dll libCryptoPP_NV_2.import.a $(TESTIMPORTOBJS)
	$(CXX) -o $@ $(strip $(CXXFLAGS)) $(TESTIMPORTOBJS) -L. -lCryptoPP_NV_2.dll -lCryptoPP_NV_2.import $(LDFLAGS) $(LDLIBS)

dlltest.exe: CryptoPP_NV_2.dll $(DLLTESTOBJS)
	$(CXX) -o $@ $(strip $(CXXFLAGS)) $(DLLTESTOBJS) -L. -lCryptoPP_NV_2.dll $(LDFLAGS) $(LDLIBS)

# Some users already have a libCryptoPP_NV_2.pc. We install it if the file
# is present. If you want one, then issue 'make libCryptoPP_NV_2.pc'. Be sure
# to use/verify PREFIX and LIBDIR below after writing the file.
libCryptoPP_NV_2.pc:
	@echo '# Crypto++ package configuration file' > libCryptoPP_NV_2.pc
	@echo '' >> libCryptoPP_NV_2.pc
	@echo 'prefix=$(PREFIX)' >> libCryptoPP_NV_2.pc
	@echo 'libdir=$(LIBDIR)' >> libCryptoPP_NV_2.pc
	@echo 'includedir=$${prefix}/include' >> libCryptoPP_NV_2.pc
	@echo '' >> libCryptoPP_NV_2.pc
	@echo 'Name: Crypto++' >> libCryptoPP_NV_2.pc
	@echo 'Description: Crypto++ cryptographic library' >> libCryptoPP_NV_2.pc
	@echo 'Version: 8.2' >> libCryptoPP_NV_2.pc
	@echo 'URL: https://CryptoPP_NV_2.com/' >> libCryptoPP_NV_2.pc
	@echo '' >> libCryptoPP_NV_2.pc
	@echo 'Cflags: -I$${includedir}' >> libCryptoPP_NV_2.pc
	@echo 'Libs: -L$${libdir} -lCryptoPP_NV_2' >> libCryptoPP_NV_2.pc

# This recipe prepares the distro files
TEXT_FILES := *.h *.cpp adhoc.cpp License.txt Readme.txt Install.txt Filelist.txt Doxyfile cryptest* cryptlib* dlltest* cryptdll* *.sln *.s *.S *.vcxproj *.filters CryptoPP_NV_2.rc TestVectors/*.txt TestData/*.dat TestPrograms/*.cxx TestScripts/*.sh TestScripts/*.cmd
...
zip dist: | distclean convert
	zip -q -9 CryptoPP_NV_2$(LIB_VER).zip $(DIST_FILES)

# Build the ISO to transfer the ZIP to old distros via CDROM
.PHONY: iso
iso: | zip
ifneq ($(IS_DARWIN),0)
	$(MKDIR) $(PWD)/CryptoPP_NV_2$(LIB_VER)
	$(CP) CryptoPP_NV_2$(LIB_VER).zip $(PWD)/CryptoPP_NV_2$(LIB_VER)
	hdiutil makehybrid -iso -joliet -o CryptoPP_NV_2$(LIB_VER).iso $(PWD)/CryptoPP_NV_2$(LIB_VER)
	@-$(RM) -r $(PWD)/CryptoPP_NV_2$(LIB_VER)
else ifneq ($(IS_LINUX)$(IS_HURD),00)
	$(MKDIR) $(PWD)/CryptoPP_NV_2$(LIB_VER)
	$(CP) CryptoPP_NV_2$(LIB_VER).zip $(PWD)/CryptoPP_NV_2$(LIB_VER)
	genisoimage -q -o CryptoPP_NV_2$(LIB_VER).iso $(PWD)/CryptoPP_NV_2$(LIB_VER)
	@-$(RM) -r $(PWD)/CryptoPP_NV_2$(LIB_VER)
endif

# CryptoPP_NV_2_CPU_FREQ in GHz
CryptoPP_NV_2_CPU_FREQ ?= 0.0
.PHONY: bench benchmark benchmarks
bench benchmark benchmarks: cryptest.exe
	@-$(RM) -f benchmarks.html
	./cryptest.exe b 2 $(CryptoPP_NV_2_CPU_FREQ)
...
# Only use CryptoPP_NV_2_DATA_DIR if its not set in CXXFLAGS
ifeq ($(findstring -DCryptoPP_NV_2_DATA_DIR, $(strip $(CXXFLAGS))),)
ifneq ($(strip $(CryptoPP_NV_2_DATA_DIR)),)
validat%.o : validat%.cpp
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_DATA_DIR=\"$(CryptoPP_NV_2_DATA_DIR)\" -c) $<
bench%.o : bench%.cpp
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_DATA_DIR=\"$(CryptoPP_NV_2_DATA_DIR)\" -c) $<
...
%.dllonly.o : %.cpp
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_DLL_ONLY -c) $< -o $@

%.import.o : %.cpp
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_IMPORTS -c) $< -o $@

%.export.o : %.cpp
	$(CXX) $(strip $(CXXFLAGS) -DCryptoPP_NV_2_EXPORTS -c) $< -o $@
