...
#define CryptoPP_NV_2_MANUALLY_INSTANTIATE_TEMPLATES
...
#if defined(CryptoPP_NV_2_EXPORTS) && defined(CryptoPP_NV_2_WIN32_AVAILABLE)
# include <windows.h>
#endif
...
// Guarding based on DLL due to Clang, http://github.com/weidai11/CryptoPP_NV_2/issues/300
...
static void SetNewAndDeleteFunctionPointers()
{
	...
	while (true)
	{
		...
		PGetNewAndDelete pGetNewAndDelete = (PGetNewAndDelete)GetProcAddress(hModule, "GetNewAndDeleteForCryptoPP_NV_2");
		if (pGetNewAndDelete)
		{
		...
		PSetNewAndDelete pSetNewAndDelete = (PSetNewAndDelete)GetProcAddress(hModule, "SetNewAndDeleteFromCryptoPP_NV_2");
		if (pSetNewAndDelete)
		{
			s_pNew = &New;
			s_pDelete = &free;
