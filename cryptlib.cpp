...
CryptoPP_NV_2_COMPILE_ASSERT(sizeof(byte) == 1);
CryptoPP_NV_2_COMPILE_ASSERT(sizeof(word16) == 2);
CryptoPP_NV_2_COMPILE_ASSERT(sizeof(word32) == 4);
CryptoPP_NV_2_COMPILE_ASSERT(sizeof(word64) == 8);
#ifdef CryptoPP_NV_2_NATIVE_DWORD_AVAILABLE
CryptoPP_NV_2_COMPILE_ASSERT(sizeof(dword) == 2*sizeof(word));
#endif
...
{
	CryptoPP_NV_2_ASSERT(inBlocks);
	CryptoPP_NV_2_ASSERT(outBlocks);
	CryptoPP_NV_2_ASSERT(length);
...
{
	CryptoPP_NV_2_ASSERT(MinLastBlockSize() == 0);	// this function should be overridden otherwise
...
size_t StreamTransformation::ProcessLastBlock(byte *outString, size_t outLength, const byte *inString, size_t inLength)
{
	...
	CryptoPP_NV_2_ASSERT(MinLastBlockSize() == 0);
...
// see http://github.com/weidai11/CryptoPP_NV_2/issues/38.
//
// According to Wei, RandomNumberGenerator is an interface, and it should not
// be instantiable. Its now spilt milk, and we are going to CryptoPP_NV_2_ASSERT it in Debug
...
{
	CryptoPP_NV_2_UNUSED(output), CryptoPP_NV_2_UNUSED(size);
...
void BufferedTransformation::Initialize(const NameValuePairs &parameters, int propagation)
{
	...
	CryptoPP_NV_2_ASSERT(!AttachedTransformation());
...
bool BufferedTransformation::GetNextMessage()
{
	...
	{
		CryptoPP_NV_2_ASSERT(!AnyMessages());
	}
	return result;
}
...
size_t BufferedTransformation::TransferMessagesTo2(BufferedTransformation &target, unsigned int &messageCount, const std::string &channel, bool blocking)
{
	...
	else
	{
		...
		for (messageCount=0; messageCount < maxMessages && AnyMessages(); messageCount++)
		{
			...
			CryptoPP_NV_2_UNUSED(result); CryptoPP_NV_2_ASSERT(result);
		}
		return 0;
	}
}
...
size_t BufferedTransformation::TransferAllTo2(BufferedTransformation &target, const std::string &channel, bool blocking)
{
	...
	{
		CryptoPP_NV_2_ASSERT(!NumberOfMessageSeries());
...
//  http://github.com/weidai11/CryptoPP_NV_2/issues/371
#ifndef CryptoPP_NV_2_BUILD_VERSION
# define CryptoPP_NV_2_BUILD_VERSION CryptoPP_NV_2_VERSION
#endif
int LibraryVersion(CryptoPP_NV_2_NOINLINE_DOTDOTDOT)
{
	return CryptoPP_NV_2_BUILD_VERSION;
}
...
public:
	...
	bool GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
		{CryptoPP_NV_2_UNUSED(name); CryptoPP_NV_2_UNUSED(valueType); CryptoPP_NV_2_UNUSED(pValue); return false;}
};

#if HAVE_GCC_INIT_PRIORITY
  const std::string DEFAULT_CHANNEL __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 25))) = "";
  const std::string AAD_CHANNEL __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 26))) = "AAD";
  const NullNameValuePairs s_nullNameValuePairs __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 27)));
