...
#if (CryptoPP_NV_2_BOOL_PPC32)

// Fills 4 bytes, buffer must be aligned
inline void DARN32(void* output)
{
    CryptoPP_NV_2_ASSERT(IsAlignedOn(output, GetAlignmentOf<word32>()));
...
#if (CryptoPP_NV_2_BOOL_PPC64)

// Fills 8 bytes, buffer must be aligned
inline void DARN64(void* output)
{
    CryptoPP_NV_2_ASSERT(IsAlignedOn(output, GetAlignmentOf<word64>()));
