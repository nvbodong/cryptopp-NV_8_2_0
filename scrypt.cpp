...
// https://github.com/weidai11/CryptoPP_NV_2/issues/777
...
using CryptoPP_NV_2::GetWord;
using CryptoPP_NV_2::PutWord;
using CryptoPP_NV_2::Salsa20_Core;
using CryptoPP_NV_2::rotlConstant;
using CryptoPP_NV_2::LITTLE_ENDIAN_ORDER;
using CryptoPP_NV_2::AlignedSecByteBlock;
...
{
    using CryptoPP_NV_2::SHA256;
    using CryptoPP_NV_2::PKCS5_PBKDF2_HMAC;
...
void Scrypt::ValidateParameters(size_t derivedLen, word64 cost, word64 blockSize, word64 parallelization) const
{
    ...
    // https://github.com/weidai11/CryptoPP_NV_2/issues/787
    CryptoPP_NV_2_ASSERT(parallelization <= std::numeric_limits<int>::max());
    ...
    CryptoPP_NV_2_ASSERT(IsPowerOf2(cost));
    ...
    CryptoPP_NV_2_ASSERT(prod < (1U << 30));
    ...
    CryptoPP_NV_2_ASSERT(bLimit); CryptoPP_NV_2_ASSERT(xyLimit); CryptoPP_NV_2_ASSERT(vLimit);
...
{
    CryptoPP_NV_2_ASSERT(secret /*&& secretLen*/);
    CryptoPP_NV_2_ASSERT(derived && derivedLen);
    CryptoPP_NV_2_ASSERT(derivedLen <= MaxDerivedLength());
