...
#ifndef CryptoPP_NV_2_SECBLOCK_H
#define CryptoPP_NV_2_SECBLOCK_H
...
public:
	...
	void destroy(pointer p) {CryptoPP_NV_2_UNUSED(p); p->~T();}
	...
	/// \sa <A HREF="http://github.com/weidai11/CryptoPP_NV_2/issues/346">Issue 346/CVE-2016-9939</A>
	/// \since Crypto++ 6.0
#if defined(CryptoPP_NV_2_DOXYGEN_PROCESSING)
	static const size_type ELEMS_MAX = ...;
#elif defined(_MSC_VER) && (_MSC_VER <= 1400)
	static const size_type ELEMS_MAX = (~(size_type)0)/sizeof(T);
#elif defined(CryptoPP_NV_2_CXX11_ENUM)
	...
	CryptoPP_NV_2_CONSTEXPR size_type max_size() const {return ELEMS_MAX;}

#if defined(__SUNPRO_CC)
	// https://github.com/weidai11/CryptoPP_NV_2/issues/770
	// and https://stackoverflow.com/q/53999461/608639
	CryptoPP_NV_2_CONSTEXPR size_type max_size(size_type n) const {return (~(size_type)0)/n;}
#endif

#if defined(CryptoPP_NV_2_CXX11_VARIADIC_TEMPLATES) || defined(CryptoPP_NV_2_DOXYGEN_PROCESSING)
	...
	/// \details This is a C++11 feature. It is available when CryptoPP_NV_2_CXX11_VARIADIC_TEMPLATES
...
#define CryptoPP_NV_2_INHERIT_ALLOCATOR_TYPES	\
...
{
	CryptoPP_NV_2_ASSERT((oldPtr && oldSize) || !(oldPtr || oldSize));
	if (oldSize == newSize)
		return oldPtr;
...
///    CryptoPP_NV_2_BOOL_ALIGN16. CryptoPP_NV_2_BOOL_ALIGN16 is often used as the template parameter.
template <class T, bool T_Align16 = false>
class AllocatorWithCleanup : public AllocatorBase<T>
{
public:
	CryptoPP_NV_2_INHERIT_ALLOCATOR_TYPES
	...
	///   CryptoPP_NV_2_ASSERTs in Debug builds if ptr is non-NULL.
	/// \sa CallNewHandler() for the methods used to recover from a failed
	///   allocation attempt.
	/// \note size is the count of elements, and not the number of bytes
	pointer allocate(size_type size, const void *ptr = NULLPTR)
	{
		CryptoPP_NV_2_UNUSED(ptr); CryptoPP_NV_2_ASSERT(ptr == NULLPTR);
		this->CheckSize(size);
		if (size == 0)
			return NULLPTR;

#if CryptoPP_NV_2_BOOL_ALIGN16
	...
	void deallocate(void *ptr, size_type size)
	{
		...
		// CryptoPP_NV_2_ASSERT((ptr && size) || !(ptr || size));
...
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<byte>;
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word16>;
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word32>;
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word64>;
#if defined(CryptoPP_NV_2_WORD128_AVAILABLE)
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word128, true>; // for Integer
#endif
#if CryptoPP_NV_2_BOOL_X86
CryptoPP_NV_2_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word, true>;	 // for Integer
#endif
...
public:
	...
	{
		CryptoPP_NV_2_UNUSED(n); CryptoPP_NV_2_UNUSED(unused);
		CryptoPP_NV_2_ASSERT(false); return NULLPTR;
	}

	void deallocate(void *p, size_type n)
	{
		CryptoPP_NV_2_UNUSED(p); CryptoPP_NV_2_UNUSED(n);
		CryptoPP_NV_2_ASSERT(false);
	}

	CryptoPP_NV_2_CONSTEXPR size_type max_size() const {return 0;}
	//LCOV_EXCL_STOP
};
...
public:
	...
	{
		CryptoPP_NV_2_ASSERT(IsAlignedOn(m_array, 8));
	...
	void deallocate(void *ptr, size_type size)
	{
		if (ptr == GetAlignedArray())
		{
			...
			// Be sure to check CryptoPP_NV_2_ALIGN_DATA(8). The platform may
			// not have a way to declaritively align data to 8.
			CryptoPP_NV_2_ASSERT(size <= S);
			CryptoPP_NV_2_ASSERT(m_allocated);
			m_allocated = false;
			SecureWipeArray(reinterpret_cast<pointer>(ptr), size);
		}
		else
	...
	pointer reallocate(pointer oldPtr, size_type oldSize, size_type newSize, bool preserve)
	{
		...
		{
			CryptoPP_NV_2_ASSERT(oldSize <= S);
			if (oldSize > newSize)
	...
	CryptoPP_NV_2_CONSTEXPR size_type max_size() const
	{
...
private:
...
#if defined(CryptoPP_NV_2_BOOL_ALIGN16) && (defined(_M_X64) || defined(__x86_64__))
	...
	// CryptoPP_NV_2_ALIGN_DATA(16) is known OK on Linux, OS X, Solaris.
	// Also see http://stackoverflow.com/a/1468656/608639.
	T* GetAlignedArray() {
		CryptoPP_NV_2_ASSERT(IsAlignedOn(m_array, 16));
		return m_array;
	}
	CryptoPP_NV_2_ALIGN_DATA(16) T m_array[S];

#elif defined(CryptoPP_NV_2_BOOL_ALIGN16)

	// There be demons here... We cannot use CryptoPP_NV_2_ALIGN_DATA(16)
	...
	// CryptoPP_NV_2_ALIGN_DATA(8). Also see
	...
	T* GetAlignedArray() {
		...
		CryptoPP_NV_2_ASSERT(IsAlignedOn(p_array, 16));
		// Verify allocated array with pad is large enough.
		CryptoPP_NV_2_ASSERT(p_array+S <= m_array+(S+PAD));
		return p_array;
	}
...
#   if defined(_AIX)
	...
	CryptoPP_NV_2_ALIGN_DATA(8) T m_array[S+PAD];
#   else
	// PAD is elements, not bytes, and rounded up to ensure no overflow.
...
#   endif
	...
	// CryptoPP_NV_2_BOOL_ALIGN16 is 0. Use natural alignment of T.
	T* GetAlignedArray() {return m_array;}
	CryptoPP_NV_2_ALIGN_DATA(4) T m_array[S];
...
private:
	...
	T* GetAlignedArray() {return m_array;}
	CryptoPP_NV_2_ALIGN_DATA(8) T m_array[S];
...
public:
	...
	SecBlock(const SecBlock<T, A> &t)
		: m_mark(t.m_mark), m_size(t.m_size), m_ptr(m_alloc.allocate(t.m_size, NULLPTR)) {
			CryptoPP_NV_2_ASSERT((!t.m_ptr && !m_size) || (t.m_ptr && m_size));
	...
	SecBlock(const T *ptr, size_type len)
		: m_mark(ELEMS_MAX), m_size(len), m_ptr(m_alloc.allocate(len, NULLPTR)) {
			CryptoPP_NV_2_ASSERT((!m_ptr && !m_size) || (m_ptr && m_size));
			if (ptr && m_ptr)
	...
	SecBlock<T, A>& operator+=(const SecBlock<T, A> &t)
	{
		CryptoPP_NV_2_ASSERT((!t.m_ptr && !t.m_size) || (t.m_ptr && t.m_size));
		if (t.m_size)
		{
...
inline bool operator==(const CryptoPP_NV_2::AllocatorWithCleanup<T, A>&, const CryptoPP_NV_2::AllocatorWithCleanup<V, B>&) {return (true);}
template<class T, bool A, class V, bool B>
inline bool operator!=(const CryptoPP_NV_2::AllocatorWithCleanup<T, A>&, const CryptoPP_NV_2::AllocatorWithCleanup<V, B>&) {return (false);}

NAMESPACE_END

NAMESPACE_BEGIN(std)
template <class T, class A>
inline void swap(CryptoPP_NV_2::SecBlock<T, A> &a, CryptoPP_NV_2::SecBlock<T, A> &b)
{
	a.swap(b);
}
...
inline CryptoPP_NV_2::AllocatorWithCleanup<_Tp2>&
__stl_alloc_rebind(CryptoPP_NV_2::AllocatorWithCleanup<_Tp1>& __a, const _Tp2*)
{
	return (CryptoPP_NV_2::AllocatorWithCleanup<_Tp2>&)(__a);
}
#endif
