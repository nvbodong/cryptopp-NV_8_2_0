...
#if defined(_MSC_VER) && !CryptoPP_NV_2_BOOL_SLOW_WORD64
#include <intrin.h>
#endif

#if defined(CryptoPP_NV_2_DISABLE_VMAC_ASM)
...
#if defined(CryptoPP_NV_2_WORD128_AVAILABLE) && !defined(CryptoPP_NV_2_X64_ASM_AVAILABLE)
using CryptoPP_NV_2::word128;
...
void VMAC_Base::UncheckedSetKey(const byte *userKey, unsigned int keylength, const NameValuePairs &params)
{
	...
	CryptoPP_NV_2_ASSERT(IsAlignedOn(l3Key,GetAlignmentOf<word64>()));
...
{
	CryptoPP_NV_2_ASSERT(IsAlignedOn(m_polyState(),GetAlignmentOf<word64>()));
	CryptoPP_NV_2_ASSERT(IsAlignedOn(m_nhKey(),GetAlignmentOf<word64>()));
	...
	CryptoPP_NV_2_UNUSED(data); CryptoPP_NV_2_UNUSED(tagPart); CryptoPP_NV_2_UNUSED(L1KeyLength);
	CryptoPP_NV_2_UNUSED(blocksRemainingInWord64);
...
#undef k3
	...
	#endif
	#if defined(CryptoPP_NV_2_X64_ASM_AVAILABLE)
	...
	#elif defined(_MSC_VER) && !CryptoPP_NV_2_BOOL_SLOW_WORD64
		#define DeclareNH(a) word64 a##0=0, a##1=0
...
{
	CryptoPP_NV_2_ASSERT(IsAlignedOn(DataBuf(),GetAlignmentOf<word64>()));
	CryptoPP_NV_2_ASSERT(IsAlignedOn(m_polyState(),GetAlignmentOf<word64>()));
