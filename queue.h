...
#ifndef CryptoPP_NV_2_QUEUE_H
#define CryptoPP_NV_2_QUEUE_H
...
class CryptoPP_NV_2_DLL ByteQueue : public Bufferless<BufferedTransformation>
{
public:
...
class CryptoPP_NV_2_DLL LazyPutter
{
public:
	LazyPutter(ByteQueue &bq, const byte *inString, size_t size)
		: m_bq(bq) {bq.LazyPut(inString, size);}
	~LazyPutter()
		{try {m_bq.FinalizeLazyPut();} catch(const Exception&) {CryptoPP_NV_2_ASSERT(0);}}
protected:
...
#ifndef __BORLANDC__
NAMESPACE_BEGIN(std)
template<> inline void swap(CryptoPP_NV_2::ByteQueue &a, CryptoPP_NV_2::ByteQueue &b)
{
	a.swap(b);
}
NAMESPACE_END
