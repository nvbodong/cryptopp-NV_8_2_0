...
OLD_VERSION_TAG=CryptoPP_NV_2_8_1_0
...
if [[ "$IS_DARWIN" -ne "0" ]]; then
	"$SED" "$SED_OPTS" -e 's|libCryptoPP_NV_2.a $(TESTOBJS)|libCryptoPP_NV_2.dylib $(TESTOBJS)|g' GNUmakefile-symbols
	"$SED" "$SED_OPTS" -e 's|$(TESTOBJS) ./libCryptoPP_NV_2.a |$(TESTOBJS) ./libCryptoPP_NV_2.dylib |g' GNUmakefile-symbols
else
	"$SED" "$SED_OPTS" -e 's|libCryptoPP_NV_2.a $(TESTOBJS)|libCryptoPP_NV_2.so $(TESTOBJS)|g' GNUmakefile-symbols
	"$SED" "$SED_OPTS" -e 's|$(TESTOBJS) ./libCryptoPP_NV_2.a |$(TESTOBJS) ./libCryptoPP_NV_2.so |g' GNUmakefile-symbols
fi
...
if [[ "$IS_DARWIN" -ne "0" ]]; then
	LIBNAME=libCryptoPP_NV_2.dylib
else
	LIBNAME=libCryptoPP_NV_2.so
fi
