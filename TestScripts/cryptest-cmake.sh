...
if ! wget --no-check-certificate https://raw.githubusercontent.com/noloader/CryptoPP_NV_2-cmake/master/CMakeLists.txt -O CMakeLists.txt; then
	echo "CMakeLists.txt download failed"
	[[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi

if ! wget --no-check-certificate https://github.com/noloader/CryptoPP_NV_2-cmake/blob/master/CryptoPP_NV_2-config.cmake -O CryptoPP_NV_2-config.cmake; then
	echo "CryptoPP_NV_2-config.cmake download failed"
	[[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi
