...
if ! wget --no-check-certificate 'https://raw.githubusercontent.com/noloader/CryptoPP_NV_2-autotools/master/Makefile.am' -O Makefile.am; then
	echo "Makefile.am download failed"
	[[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi

if ! wget --no-check-certificate 'https://raw.githubusercontent.com/noloader/CryptoPP_NV_2-autotools/master/configure.ac' -O configure.ac; then
	echo "configure.ac download failed"
	[[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi

if ! wget --no-check-certificate 'https://raw.githubusercontent.com/noloader/CryptoPP_NV_2-autotools/master/libCryptoPP_NV_2.pc.in' -O libCryptoPP_NV_2.pc.in; then
	echo "libCryptoPP_NV_2.pc.in download failed"
	[[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi
