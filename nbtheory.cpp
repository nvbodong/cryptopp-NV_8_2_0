...
bool TrialDivision(const Integer &p, unsigned bound)
{
	...
	CryptoPP_NV_2_ASSERT(primeTable[primeTableSize-1] >= bound);
...
bool IsFermatProbablePrime(const Integer &n, const Integer &b)
{
	...
	CryptoPP_NV_2_ASSERT(n>3 && b>1 && b<n-1);
...
bool PrimeSieve::NextCandidate(Integer &c)
{
	...
	CryptoPP_NV_2_UNUSED(safe); CryptoPP_NV_2_ASSERT(safe);
...
void PrimeSieve::DoSieve()
{
	...
	{
		CryptoPP_NV_2_ASSERT(m_step%2==0);
...
{
	CryptoPP_NV_2_ASSERT(!equiv.IsNegative() && equiv < mod);
	...
	CryptoPP_NV_2_ASSERT(p > primeTable[primeTableSize-1]);
...
{
	CryptoPP_NV_2_ASSERT(p < q*q*q);
	CryptoPP_NV_2_ASSERT(p % q == 1);
...
// which holds iff p is built up of two factors (exercise ... )
	...
	CryptoPP_NV_2_ASSERT(primeTableSize >= 50);
	for (int i=0; i<50; i++)
	{
...
Integer ModularSquareRoot(const Integer &a, const Integer &p)
{
	...
	CryptoPP_NV_2_ASSERT(x.Squared()%p == a);
	return x;
}
...
bool SolveModularQuadraticEquation(Integer &r1, Integer &r2, const Integer &a, const Integer &b, const Integer &c, const Integer &p)
{
	...
	{
	default:
		CryptoPP_NV_2_ASSERT(false);	// not reached
		return false;
	case -1:
		return false;
	case 0:
		r1 = r2 = (-b*(a+a).InverseMod(p)) % p;
		CryptoPP_NV_2_ASSERT(((r1.Squared()*a + r1*b + c) % p).IsZero());
		return true;
	case 1:
		...
		r2 = (-s-b)*t % p;
		CryptoPP_NV_2_ASSERT(((r1.Squared()*a + r1*b + c) % p).IsZero());
		CryptoPP_NV_2_ASSERT(((r2.Squared()*a + r2*b + c) % p).IsZero());
		return true;
	}
}
...
{
	CryptoPP_NV_2_ASSERT(b.Positive());
...
{
	CryptoPP_NV_2_ASSERT(bIn.IsOdd());
...
void PrimeAndGenerator::Generate(signed int delta, RandomNumberGenerator &rng, unsigned int pbits, unsigned int qbits)
{
	...
	CryptoPP_NV_2_ASSERT(qbits > 4);
	CryptoPP_NV_2_ASSERT(pbits > qbits);
	...
	if (qbits+1 == pbits)
	{
		...
		while (!success)
		{
			...
			while (sieve.NextCandidate(p))
			{
				CryptoPP_NV_2_ASSERT(IsSmallPrime(p) || SmallDivisorsTest(p));
				q = (p-delta) >> 1;
				CryptoPP_NV_2_ASSERT(IsSmallPrime(q) || SmallDivisorsTest(q));
		...
		if (delta == 1)
		{
			...
			CryptoPP_NV_2_ASSERT((p%8==1 || p%8==7) ? g==2 : (p%12==1 || p%12==11) ? g==3 : g==4);
		}
		else
		{
			CryptoPP_NV_2_ASSERT(delta == -1);
	...
	else
	{
		...
		if (delta==1)
		{
			...
			CryptoPP_NV_2_ASSERT(a_exp_b_mod_c(g, q, p)==1);
		}
		else
		{
			CryptoPP_NV_2_ASSERT(delta==-1);
			...
			CryptoPP_NV_2_ASSERT(Lucas(q, g, p) == 2);
		}
	}
}
