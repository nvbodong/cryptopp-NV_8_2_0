...
#ifndef CryptoPP_NV_2_VALIDATE_H
#define CryptoPP_NV_2_VALIDATE_H
...
// If CryptoPP_NV_2_DEBUG or CryptoPP_NV_2_COVERAGE is in effect, then perform additional tests
#if (defined(CryptoPP_NV_2_DEBUG) || defined(CryptoPP_NV_2_COVERAGE) || defined(CryptoPP_NV_2_VALGRIND)) && !defined(CryptoPP_NV_2_IMPORTS)
# define CryptoPP_NV_2_EXTENDED_VALIDATION 1
#endif
...
bool TestASN1Parse();
// https://github.com/weidai11/CryptoPP_NV_2/pull/334
bool TestStringSink();
...
// Use CryptoPP_NV_2_DATA_DIR last. The problem this sidesteps is, finding an
// old version of Crypto++ library in CryptoPP_NV_2_DATA_DIR when the library
// has been staged in DESTDIR. Using CryptoPP_NV_2_DATA_DIR first only works
// as expected when CryptoPP_NV_2_DATA_DIR is empty before an install. We
...
static std::string GetDataDir()
{
	...
	name = AddSeparator(g_argvPathHint) + std::string("../share/CryptoPP_NV_2/") + filename;
	file.open(name.c_str());
	if (file.is_open())
		return AddSeparator(g_argvPathHint) + std::string("../share/CryptoPP_NV_2/");
#endif
#ifndef CryptoPP_NV_2_DISABLE_DATA_DIR_SEARCH
...
#endif
#ifdef CryptoPP_NV_2_DATA_DIR
	// Honor CryptoPP_NV_2_DATA_DIR. This is likely an install directory if it is not "./".
	name = AddSeparator(CryptoPP_NV_2_DATA_DIR) + filename;
	file.open(name.c_str());
	if (file.is_open())
		return AddSeparator(CryptoPP_NV_2_DATA_DIR);
#endif
	return "./";
}
...
#if CryptoPP_NV_2_CXX11_DYNAMIC_INIT
