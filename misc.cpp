...
// for memalign
#if defined(CryptoPP_NV_2_MEMALIGN_AVAILABLE) || defined(CryptoPP_NV_2_MM_MALLOC_AVAILABLE) || defined(QNX)
# include <malloc.h>
#endif
// for posix_memalign
#if defined(CryptoPP_NV_2_POSIX_MEMALIGN_AVAILABLE)
# include <stdlib.h>
#endif
...
{
	CryptoPP_NV_2_ASSERT(buf != NULLPTR);
	CryptoPP_NV_2_ASSERT(mask != NULLPTR);
	CryptoPP_NV_2_ASSERT(count > 0);

	size_t i=0;
	if (IsAligned<word32>(buf) && IsAligned<word32>(mask))
	{
		if (!CryptoPP_NV_2_BOOL_SLOW_WORD64 && IsAligned<word64>(buf) && IsAligned<word64>(mask))
		{
			for (i=0; i<count/8; i++)
...
{
	CryptoPP_NV_2_ASSERT(output != NULLPTR);
	CryptoPP_NV_2_ASSERT(input != NULLPTR);
	CryptoPP_NV_2_ASSERT(count > 0);

	size_t i=0;
	if (IsAligned<word32>(output) && IsAligned<word32>(input) && IsAligned<word32>(mask))
	{
		if (!CryptoPP_NV_2_BOOL_SLOW_WORD64 && IsAligned<word64>(output) && IsAligned<word64>(input) && IsAligned<word64>(mask))
		{
			for (i=0; i<count/8; i++)
...
{
	CryptoPP_NV_2_ASSERT(str);
	std::string result;

	// Safer functions on Windows for C&A, https://github.com/weidai11/CryptoPP_NV_2/issues/55
#if (CryptoPP_NV_2_MSC_VERSION >= 1400)
	size_t len=0, size=0;
	errno_t err = 0;
	...
	CryptoPP_NV_2_ASSERT(size != (size_t)-1);
	if (size == (size_t)-1)
	{
		if (throwOnError)
...
{
	byte *p;
#if defined(CryptoPP_NV_2_MM_MALLOC_AVAILABLE)
	while ((p = (byte *)_mm_malloc(size, 16)) == NULLPTR)
#elif defined(CryptoPP_NV_2_MEMALIGN_AVAILABLE)
	while ((p = (byte *)memalign(16, size)) == NULLPTR)
#elif defined(CryptoPP_NV_2_MALLOC_ALIGNMENT_IS_16)
	while ((p = (byte *)malloc(size)) == NULLPTR)
#elif defined(CryptoPP_NV_2_POSIX_MEMALIGN_AVAILABLE)
...
#ifdef CryptoPP_NV_2_NO_ALIGNED_ALLOC
	size_t adjustment = 16-((size_t)p%16);
	CryptoPP_NV_2_ASSERT(adjustment > 0);
	p += adjustment;
	p[-1] = (byte)adjustment;
#endif
	...
	CryptoPP_NV_2_ASSERT(IsAlignedOn(p, 16));
	return p;
}

void AlignedDeallocate(void *p)
{
#ifdef CryptoPP_NV_2_MM_MALLOC_AVAILABLE
	_mm_free(p);
#elif defined(CryptoPP_NV_2_NO_ALIGNED_ALLOC)
	p = (byte *)p - ((byte *)p)[-1];
	free(p);
#else
	free(p);
