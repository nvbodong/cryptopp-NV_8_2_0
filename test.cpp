...
//            CryptoPP_NV_2::Test namespace added by JW in February 2017
//            scoped_main added to CryptoPP_NV_2::Test namespace by JW in July 2017
//            Also see http://github.com/weidai11/CryptoPP_NV_2/issues/447

#define CryptoPP_NV_2_DEFAULT_NO_DLL
...
#if defined(CryptoPP_NV_2_UNIX_AVAILABLE) || defined(CryptoPP_NV_2_BSD_AVAILABLE)
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
#if defined(CryptoPP_NV_2_OSX_AVAILABLE)
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
// If CryptoPP_NV_2_USE_AES_GENERATOR is 1 then AES/OFB based is used.
// Otherwise the OS random number generator is used.
#define CryptoPP_NV_2_USE_AES_GENERATOR 1

// Global namespace, provided by other source files
void FIPS140_SampleApplication();
void RegisterFactories(CryptoPP_NV_2::Test::TestClass suites);
...
#if (CryptoPP_NV_2_USE_AES_GENERATOR)
...
int scoped_main(int argc, char *argv[])
{
	...
	try
	{
		...
		//  interface, to key the underlying cipher. If CryptoPP_NV_2_USE_AES_GENERATOR is 1
		...
		else if (command == "mac_dll")
		{
			...
			byte mac[] = CryptoPP_NV_2_DUMMY_DLL_MAC;
			...
			CryptoPP_NV_2_ASSERT(pMac->DigestSize() == sizeof(mac));
		...
		{
			// TestDataFile() adds CryptoPP_NV_2_DATA_DIR as required
		...
		{
			std::cout << CryptoPP_NV_2_VERSION / 100 << '.' << (CryptoPP_NV_2_VERSION % 100) / 10 << '.' << CryptoPP_NV_2_VERSION % 10 << std::endl;
		}
		else
		{
...
# if (PATH_MAX > 0)  // Posix
	...
	if (err == 0 && pgmptr != NULLPTR)
		pathHint = pgmptr;
#elif defined(CryptoPP_NV_2_OSX_AVAILABLE)
...
void SecretShareFile(int threshold, int nShares, const char *filename, const char *seed)
{
	CryptoPP_NV_2_ASSERT(nShares >= 1 && nShares<=1000);
	...
	//    is full. Also see http://groups.google.com/forum/#!topic/CryptoPP_NV_2-users/XEKKLCEFH3Y.
...
void SecretRecoverFile(int threshold, const char *outFilename, char *const *inFilenames)
{
	CryptoPP_NV_2_ASSERT(threshold >= 1 && threshold <=1000);
...
void InformationRecoverFile(int threshold, const char *outFilename, char *const *inFilenames)
{
	CryptoPP_NV_2_ASSERT(threshold<=1000);
	if (threshold < 1 || threshold > 1000)
...
//	case 30: result = ValidateXMACC(); break;
...
#if defined(CryptoPP_NV_2_EXTENDED_VALIDATION)
	// http://github.com/weidai11/CryptoPP_NV_2/issues/92
	case 9999: result = TestSecBlock(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/64
	case 9998: result = TestPolynomialMod2(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/336
	...
	// http://github.com/weidai11/CryptoPP_NV_2/issues/360
	case 9995: result = TestRounding(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/242
	case 9994: result = TestHuffmanCodes(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/346
	case 9993: result = TestASN1Parse(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/242
	case 9992: result = TestX25519(); break;
	// http://github.com/weidai11/CryptoPP_NV_2/issues/346
	case 9991: result = TestEd25519(); break;
# if defined(CryptoPP_NV_2_ALTIVEC_AVAILABLE)
...
// a 'using namespace CryptoPP_NV_2', it causes compile failures.
// Also see http://github.com/weidai11/CryptoPP_NV_2/issues/442
// and http://github.com/weidai11/CryptoPP_NV_2/issues/447.
int CryptoPP_NV_2_API main(int argc, char *argv[])
{
	return CryptoPP_NV_2::Test::scoped_main(argc, argv);
}
