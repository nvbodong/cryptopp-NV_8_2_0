...
#ifndef CryptoPP_NV_2_PKCSPAD_CPP	// SunCC workaround: compiler could cause this file to be included twice
#define CryptoPP_NV_2_PKCSPAD_CPP
...
// Typedef/cast change due to Clang, http://github.com/weidai11/CryptoPP_NV_2/issues/300
...
// http://github.com/weidai11/CryptoPP_NV_2/issues/517. OIDs and encoded prefixes found at
...
void PKCS_EncryptionPaddingScheme::Pad(RandomNumberGenerator& rng, const byte *input, size_t inputLen, byte *pkcsBlock, size_t pkcsBlockLen, const NameValuePairs& parameters) const
{
	...
	CryptoPP_NV_2_ASSERT (inputLen <= MaxUnpaddedLength(pkcsBlockLen));	// this should be checked by caller
...
DecodingResult PKCS_EncryptionPaddingScheme::Unpad(const byte *pkcsBlock, size_t pkcsBlockLen, byte *output, const NameValuePairs& parameters) const
{
	...
	CryptoPP_NV_2_ASSERT(i==pkcsBlockLen || pkcsBlock[i-1]==0);
