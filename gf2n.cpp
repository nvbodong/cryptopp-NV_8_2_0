...
using CryptoPP_NV_2::PolynomialMod2;

#if defined(HAVE_GCC_INIT_PRIORITY)
  const PolynomialMod2 g_zero __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 60))) = PolynomialMod2();
  const PolynomialMod2 g_one __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 61))) = PolynomialMod2(1);
...
extern CryptoPP_NV_2_DLL void GF2NT_233_Multiply_Reduce_CLMUL(const word* pA, const word* pB, word* pC);
extern CryptoPP_NV_2_DLL void GF2NT_233_Square_Reduce_CLMUL(const word* pA, word* pC);
#endif
...
{
	CryptoPP_NV_2_ASSERT(value==0 || reg.size()>0);
...
unsigned int PolynomialMod2::Parity() const
{
	...
	for (i=0; i<reg.size(); i++)
		temp ^= reg[i];
	return CryptoPP_NV_2::Parity(temp);
}
...
PolynomialMod2& PolynomialMod2::operator<<=(unsigned int n)
{
	...
	int x=0; CryptoPP_NV_2_UNUSED(x);
	CryptoPP_NV_2_ASSERT(SafeConvert(n,x));
#endif
	...
	if (carry)
	{
		// Thanks to Apatryda, http://github.com/weidai11/CryptoPP_NV_2/issues/64
...
GF2NT::GF2NT(unsigned int c0, unsigned int c1, unsigned int c2)
	...
	, result((word)0, m)
{
	CryptoPP_NV_2_ASSERT(c0 > c1 && c1 > c2 && c2==0);
}
...
const GF2NT::Element& GF2NT::MultiplicativeInverse(const Element &a) const
{
	...
	b[0]=1;
	CryptoPP_NV_2_ASSERT(a.reg.size() <= m_modulus.reg.size());
	...
	while (1)
	{
		...
		while (!t)
		{
			...
			if (c[bcLen-1])
				bcLen++;
			CryptoPP_NV_2_ASSERT(bcLen <= m_modulus.reg.size());
	...
	while (k >= WORD_BITS)
	{
		...
		if (t1 < WORD_BITS)
			for (unsigned int j=0; j<WORD_BITS-t1; j++)
			{
				...
				CryptoPP_NV_2_ASSERT(shift < WORD_BITS);
	...
	if (i==BitsToWords(t0)-1 && t0%WORD_BITS)
	{
		...
		if ((t0-t1)%WORD_BITS)
		{
			...
			if ((t0-t1)%WORD_BITS > t0%WORD_BITS)
				...
				CryptoPP_NV_2_ASSERT(temp << (WORD_BITS - (t0-t1)%WORD_BITS) == 0);
		}
		else
			b[i-(t0-t1)/WORD_BITS] ^= temp;
	}
...
const GF2NT::Element& GF2NT233::Multiply(const Element &a, const Element &b) const
{
	...
	if (HasCLMUL())
	{
		CryptoPP_NV_2_ASSERT(a.reg.size()*WORD_BITS == 256);
		CryptoPP_NV_2_ASSERT(b.reg.size()*WORD_BITS == 256);
		CryptoPP_NV_2_ASSERT(result.reg.size()*WORD_BITS == 256);
