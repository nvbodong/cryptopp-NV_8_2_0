...
void AddHtmlHeader()
{
	...
	oss << "\n<H1><A href=\"http://www.CryptoPP_NV_2.com\">Crypto++</A> " << CryptoPP_NV_2_VERSION / 100;
	oss << '.' << (CryptoPP_NV_2_VERSION % 100) / 10 << '.' << CryptoPP_NV_2_VERSION % 10 << " Benchmarks</H1>";
...
#if (CryptoPP_NV_2_BOOL_X86) && !defined(CryptoPP_NV_2_DISABLE_ASM)
		if (HasPadlockRNG())
			BenchMarkByNameKeyLess<RandomNumberGenerator>("PadlockRNG");
#endif
#if (CryptoPP_NV_2_BOOL_X86 || CryptoPP_NV_2_BOOL_X32 || CryptoPP_NV_2_BOOL_X64) && !defined(CryptoPP_NV_2_DISABLE_ASM)
		if (HasRDRAND())
			BenchMarkByNameKeyLess<RandomNumberGenerator>("RDRAND");
		if (HasRDSEED())
			BenchMarkByNameKeyLess<RandomNumberGenerator>("RDSEED");
#endif
#if (CryptoPP_NV_2_BOOL_PPC32 || CryptoPP_NV_2_BOOL_PPC64) && !defined(CryptoPP_NV_2_DISABLE_ASM)
		if (HasDARN())
