...
// "Error: The operand ___LKDB cannot be assigned to", http://github.com/weidai11/CryptoPP_NV_2/issues/188
...
# undef CryptoPP_NV_2_X86_ASM_AVAILABLE
# undef CryptoPP_NV_2_X32_ASM_AVAILABLE
# undef CryptoPP_NV_2_X64_ASM_AVAILABLE
# undef CryptoPP_NV_2_SSE2_ASM_AVAILABLE
# undef CryptoPP_NV_2_SSSE3_ASM_AVAILABLE
#else
# define CryptoPP_NV_2_INTEGER_SSE2 (CryptoPP_NV_2_SSE2_ASM_AVAILABLE && (CryptoPP_NV_2_BOOL_X86))
#endif
...
// http://github.com/weidai11/CryptoPP_NV_2/issues/707.
...
static word AtomicInverseModPower2(word A)
{
	...
	CryptoPP_NV_2_ASSERT(R*A==1);
	return R;
}

// ********************************************************

#if !defined(CryptoPP_NV_2_NATIVE_DWORD_AVAILABLE) || (defined(__x86_64__) && defined(CryptoPP_NV_2_WORD128_AVAILABLE))
...
{
public:
#if defined(CryptoPP_NV_2_NATIVE_DWORD_AVAILABLE)
	DWord() {std::memset(&m_whole, 0x00, sizeof(m_whole));}
#else
	DWord() {std::memset(&m_halfs, 0x00, sizeof(m_halfs));}
#endif

#ifdef CryptoPP_NV_2_NATIVE_DWORD_AVAILABLE
...
private:
	...
	//   http://github.com/weidai11/CryptoPP_NV_2/issues/274
...
{
	CryptoPP_NV_2_UNUSED(dummy);

	// Assert {A[2],A[1]} < {B1,B0}, so quotient can fit in a S
	CryptoPP_NV_2_ASSERT(A[2] < B1 || (A[2]==B1 && A[1] < B0));
	...
	while (A[2] || A[1] > B1 || (A[1]==B1 && A[0]>=B0))
	{
		...
		A[2] += u.GetHighHalf();
		Q++;
		CryptoPP_NV_2_ASSERT(Q);	// shouldn't overflow
	}
...
#ifdef CryptoPP_NV_2_X64_MASM_AVAILABLE
extern "C" {
int Baseline_Add(size_t N, word *C, const word *A, const word *B);
int Baseline_Sub(size_t N, word *C, const word *A, const word *B);
}
#elif defined(CryptoPP_NV_2_X64_ASM_AVAILABLE) && defined(__GNUC__) && defined(CryptoPP_NV_2_WORD128_AVAILABLE)
int Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	word result;
	__asm__ __volatile__
...
#elif defined(CryptoPP_NV_2_X86_ASM_AVAILABLE) && CryptoPP_NV_2_BOOL_X86
CryptoPP_NV_2_NAKED int CryptoPP_NV_2_FASTCALL Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
	...
	// http://github.com/weidai11/CryptoPP_NV_2/issues/340
	CryptoPP_NV_2_UNUSED(A); CryptoPP_NV_2_UNUSED(B);
	CryptoPP_NV_2_UNUSED(C); CryptoPP_NV_2_UNUSED(N);
}

CryptoPP_NV_2_NAKED int CryptoPP_NV_2_FASTCALL Baseline_Sub(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
#if CryptoPP_NV_2_INTEGER_SSE2
CryptoPP_NV_2_NAKED int CryptoPP_NV_2_FASTCALL SSE2_Add(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
CryptoPP_NV_2_NAKED int CryptoPP_NV_2_FASTCALL SSE2_Sub(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
#endif	// CryptoPP_NV_2_INTEGER_SSE2
#else   // CryptoPP_NV_2_SSE2_ASM_AVAILABLE
int CryptoPP_NV_2_FASTCALL Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	CryptoPP_NV_2_ASSERT (N%2 == 0);
...
int CryptoPP_NV_2_FASTCALL Baseline_Sub(size_t N, word *C, const word *A, const word *B)
{
	CryptoPP_NV_2_ASSERT (N%2 == 0);
...
{
	// http://github.com/weidai11/CryptoPP_NV_2/issues/188
...
#ifndef CryptoPP_NV_2_DOXYGEN_PROCESSING
...
// http://github.com/weidai11/CryptoPP_NV_2/issues/340
	...
	CryptoPP_NV_2_UNUSED(d0); CryptoPP_NV_2_UNUSED(d1);
#endif
}
...
#if !CryptoPP_NV_2_INTEGER_SSE2	// save memory by not compiling these functions when SSE2 is available
...
#endif	// #if CryptoPP_NV_2_INTEGER_SSE2

// ********************************************************

typedef int (CryptoPP_NV_2_FASTCALL * PAdd)(size_t N, word *C, const word *A, const word *B);
...
{
	CryptoPP_NV_2_ASSERT(N>=2 && N%2==0);
	...
	else
	{
		...
		CryptoPP_NV_2_ASSERT (c3 >= 0 && c3 <= 2);
		Increment(R3, N2, c3);
	}
}
...
void MontgomeryReduce(word *R, word *T, word *X, const word *M, const word *U, size_t N)
{
	...
	CryptoPP_NV_2_ASSERT(carry | !borrow);
	CryptoPP_NV_2_UNUSED(carry), CryptoPP_NV_2_UNUSED(borrow);
...
{
	CryptoPP_NV_2_ASSERT(N%2==0 && N>=4);
...
#define X3		(X+N+N2)
	...
	CryptoPP_NV_2_ASSERT(c3>=-1 && c3<=1);
	if (c3>0)
		Subtract(R, R, M, N);
	else if (c3<0)
		Add(R, R, M, N);
...
#if defined(CryptoPP_NV_2_DEBUG)
		// multiply quotient and divisor and add remainder, make sure it equals dividend
		CryptoPP_NV_2_ASSERT(!T[2] && !T[3] && (T[1] < B[1] || (T[1]==B[1] && T[0]<B[0])));
		word P[4];
		LowLevel::Multiply2(P, Q, B);
		Add(P, P, T, 4);
		CryptoPP_NV_2_ASSERT(memcmp(P, A, 4*WORD_SIZE)==0);
#endif
	}
}
*/
...
static void CorrectQuotientEstimate(word *R, word *T, word *Q, const word *B, size_t N)
{
	...
	CryptoPP_NV_2_ASSERT(!borrow && !R[N+1]);
	CryptoPP_NV_2_UNUSED(borrow);

	while (R[N] || Compare(R, B, N) >= 0)
	{
		R[N] -= Subtract(R, R, B, N);
		Q[1] += (++Q[0]==0);
		CryptoPP_NV_2_ASSERT(Q[0] || Q[1]); // no overflow
	}
}
...
{
	CryptoPP_NV_2_ASSERT(NA && NB && NA%2==0 && NB%2==0);
	CryptoPP_NV_2_ASSERT(B[NB-1] || B[NB-2]);
	CryptoPP_NV_2_ASSERT(NB <= NA);
	...
	CryptoPP_NV_2_ASSERT(shiftBits < WORD_BITS);
	...
	{
		NA+=2;
		CryptoPP_NV_2_ASSERT(Compare(TA+NA-NB, TB, NB) < 0);
	}
...
{
	CryptoPP_NV_2_ASSERT(NA<=N && N && N%2==0);
...
{
	CryptoPP_NV_2_ASSERT(IsConvertableToLong());
...
{
	CryptoPP_NV_2_ASSERT(o == BIG_ENDIAN_ORDER || o == LITTLE_ENDIAN_ORDER);
...
{
	CryptoPP_NV_2_ASSERT(encodedInteger && byteCount); // NULL buffer
	CryptoPP_NV_2_ASSERT(o == BIG_ENDIAN_ORDER || o == LITTLE_ENDIAN_ORDER);
...
{
	lword v = 0;
	CryptoPP_NV_2_ASSERT(n <= sizeof(v)*8);
...
{
	CryptoPP_NV_2_ASSERT( order == BIG_ENDIAN_ORDER || order == LITTLE_ENDIAN_ORDER );
...
{
	CryptoPP_NV_2_ASSERT(input && inputLen); // NULL buffer
...
{
	CryptoPP_NV_2_ASSERT(bt.MaxRetrievable() >= inputLen);
...
{
	CryptoPP_NV_2_ASSERT(output && outputLen);            // NULL buffer
...
{
	CryptoPP_NV_2_ASSERT(input && len); // NULL buffer
...
{
	CryptoPP_NV_2_ASSERT(output && bufferSize);            // NULL buffer
	CryptoPP_NV_2_ASSERT(bufferSize >= MinEncodedSize());  // Undersized buffer
...
public:
	...
	{
		CryptoPP_NV_2_ASSERT(output && size); // NULL buffer
...
#ifdef CryptoPP_NV_2_USE_STD_SHOWBASE
...
Integer& Integer::operator++()
{
	...
	else
	{
		...
		CryptoPP_NV_2_ASSERT(!borrow); CryptoPP_NV_2_UNUSED(borrow);
...
int Integer::PositiveCompare(const Integer& t) const
{
	...
	if (size != tSize)
		...
		return CryptoPP_NV_2::Compare(reg, t.reg, size);
}
...
{
	CryptoPP_NV_2_ASSERT(m.NotZero());
	if (m.IsZero())
		throw Integer::DivideByZero();
...
{
	CryptoPP_NV_2_ASSERT(m.NotNegative());
	CryptoPP_NV_2_ASSERT(m.NotZero());

	if (IsNegative())
		return Modulo(m).InverseModNext(m);

	// http://github.com/weidai11/CryptoPP_NV_2/issues/602
	if (*this >= m)
		return Modulo(m).InverseModNext(m);
...
const Integer& ModularArithmetic::Half(const Integer &a) const
{
	...
	{
		CryptoPP_NV_2::DivideByPower2Mod(m_result.reg.begin(), a.reg, 1, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
...
const Integer& ModularArithmetic::Add(const Integer &a, const Integer &b) const
{
	...
	{
		if (CryptoPP_NV_2::Add(m_result.reg.begin(), a.reg, b.reg, a.reg.size())
			|| Compare(m_result.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			CryptoPP_NV_2::Subtract(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		}
		return m_result;
	}
	else
...
Integer& ModularArithmetic::Accumulate(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (CryptoPP_NV_2::Add(a.reg, a.reg, b.reg, a.reg.size())
			|| Compare(a.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			CryptoPP_NV_2::Subtract(a.reg, a.reg, m_modulus.reg, a.reg.size());
		}
	}
	else
	{
...
const Integer& ModularArithmetic::Subtract(const Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (CryptoPP_NV_2::Subtract(m_result.reg.begin(), a.reg, b.reg, a.reg.size()))
			CryptoPP_NV_2::Add(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
	{
...
Integer& ModularArithmetic::Reduce(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (CryptoPP_NV_2::Subtract(a.reg, a.reg, b.reg, a.reg.size()))
			CryptoPP_NV_2::Add(a.reg, a.reg, m_modulus.reg, a.reg.size());
	}
	else
	{
		a-=b;
...
const Integer& ModularArithmetic::Inverse(const Integer &a) const
{
	...
	if (CryptoPP_NV_2::Subtract(m_result.reg.begin(), m_result.reg, a.reg, a.reg.size()))
...
const Integer& MontgomeryRepresentation::Multiply(const Integer &a, const Integer &b) const
{
	...
	CryptoPP_NV_2_ASSERT(a.reg.size()<=N && b.reg.size()<=N);
...
const Integer& MontgomeryRepresentation::Square(const Integer &a) const
{
	...
	word *const R = m_result.reg.begin();
	const size_t N = m_modulus.reg.size();
	CryptoPP_NV_2_ASSERT(a.reg.size()<=N);

	CryptoPP_NV_2::Square(T, T+2*N, a.reg, a.reg.size());
...
std::string IntToString<Integer>(Integer value, unsigned int base)
{
	...
	base &= ~(BIT_32|BIT_31);
	CryptoPP_NV_2_ASSERT(base >= 2 && base <= 32);
...
#ifndef CryptoPP_NV_2_NO_ASSIGN_TO_INTEGER
// Allow the linker to discard Integer code if not needed.
// Also see http://github.com/weidai11/CryptoPP_NV_2/issues/389.
...
#endif  // CryptoPP_NV_2_NO_ASSIGN_TO_INTEGER
...
// init priorities are not available.
	...
	const InitInteger s_init __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 10))) = InitInteger();
	const Integer g_zero __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 11))) = Integer(0L);
	const Integer g_one __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 12))) = Integer(1L);
	const Integer g_two __attribute__ ((init_priority (CryptoPP_NV_2_INIT_PRIORITY + 13))) = Integer(2L);
...
const Integer &Integer::Zero()
{
	...
	return g_zero;
#elif defined(CryptoPP_NV_2_CXX11_DYNAMIC_INIT)
