...
// #undef CryptoPP_NV_2_ALTIVEC_AVAILABLE
// #undef CryptoPP_NV_2_POWER8_AVAILABLE

// Disable NEON/ASIMD for Cortex-A53 and A57. The shifts are too slow and C/C++ is about
// 3 cpb faster than NEON/ASIMD. Also see http://github.com/weidai11/CryptoPP_NV_2/issues/367.
#if (defined(__aarch32__) || defined(__aarch64__)) && defined(CryptoPP_NV_2_SLOW_ARMV8_SHIFT)
# undef CryptoPP_NV_2_ARM_NEON_AVAILABLE
#endif

// BLAKE2s bug on AIX 7.1 (POWER7) with XLC 12.01
// https://github.com/weidai11/CryptoPP_NV_2/issues/743
#if defined(__xlC__) && (__xlC__ < 0x0d01)
# define CryptoPP_NV_2_DISABLE_ALTIVEC 1
# undef CryptoPP_NV_2_POWER7_AVAILABLE
# undef CryptoPP_NV_2_POWER8_AVAILABLE
# undef CryptoPP_NV_2_ALTIVEC_AVAILABLE
#endif
...
{
    CryptoPP_NV_2_ASSERT(digestSize <= DIGESTSIZE);
...
{
    CryptoPP_NV_2_ASSERT(keyLength <= MAX_KEYLENGTH);
    CryptoPP_NV_2_ASSERT(digestSize <= DIGESTSIZE);
    CryptoPP_NV_2_ASSERT(saltLength <= SALTSIZE);
    CryptoPP_NV_2_ASSERT(personalizationLength <= PERSONALIZATIONSIZE);
...
void BLAKE2s::UncheckedSetKey(const byte *key, unsigned int length, const CryptoPP_NV_2::NameValuePairs& params)
{
    if (key && length)
    {
        m_key.New(BLOCKSIZE);
...
void BLAKE2b::UncheckedSetKey(const byte *key, unsigned int length, const CryptoPP_NV_2::NameValuePairs& params)
{
    if (key && length)
    {
        m_key.New(BLOCKSIZE);
...
{
    CryptoPP_NV_2_ASSERT(input != NULLPTR || length == 0);
    ...
    if (length)
    {
        CryptoPP_NV_2_ASSERT(length <= BLOCKSIZE - m_state.m_len);
...
void BLAKE2b::Update(const byte *input, size_t length)
{
    ...
    if (length > BLOCKSIZE - m_state.m_len)
    {
        ...
        while (length > BLOCKSIZE)
        {
            CryptoPP_NV_2_ASSERT(m_state.m_len == 0);
            IncrementCounter(BLOCKSIZE);
            Compress(input);
            length -= BLOCKSIZE, input += BLOCKSIZE;
        }
...
{
    CryptoPP_NV_2_ASSERT(hash != NULLPTR);
