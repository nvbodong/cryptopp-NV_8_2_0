...
{
#if defined(CryptoPP_NV_2_CXX11_DYNAMIC_INIT)
...
{
	CryptoPP_NV_2_ASSERT(qbits > 9);	// no primes exist for pbits = 10, qbits = 9
...
top:
	...
	// CryptoPP_NV_2_ASSERT((p % 3U) == 2);
	// CryptoPP_NV_2_ASSERT(((p.Squared() - p + 1) % q).IsZero());
	...
	// CryptoPP_NV_2_ASSERT(XTR_Exponentiate(g, q, p) == three);
}
