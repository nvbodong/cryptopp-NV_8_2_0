...
#if defined(CryptoPP_NV_2_WIN32_AVAILABLE) && !defined(OS_RNG_AVAILABLE)
...
#ifdef CryptoPP_NV_2_UNIX_AVAILABLE
#include <errno.h>
#include <fcntl.h>
#include <unistd.h>
#endif
...
# define CryptoPP_NV_2_CONTAINER L"Crypto++ RNG"
#else
# define CryptoPP_NV_2_CONTAINER "Crypto++ RNG"
#endif
...
MicrosoftCryptoProvider::MicrosoftCryptoProvider() : m_hProvider(0)
{
	...
	if (!CryptAcquireContext(&m_hProvider, 0, 0, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT))
	{
		...
		if (!CryptAcquireContext(&m_hProvider, CryptoPP_NV_2_CONTAINER, 0, PROV_RSA_FULL, CRYPT_NEWKEYSET /*user*/) &&
		    !CryptAcquireContext(&m_hProvider, CryptoPP_NV_2_CONTAINER, 0, PROV_RSA_FULL, CRYPT_MACHINE_KEYSET|CRYPT_NEWKEYSET))
		{
...
{
#ifndef CryptoPP_NV_2_WIN32_AVAILABLE
...
#ifndef CryptoPP_NV_2_BLOCKING_RNG_FILENAME
#ifdef __OpenBSD__
#define CryptoPP_NV_2_BLOCKING_RNG_FILENAME "/dev/srandom"
#else
#define CryptoPP_NV_2_BLOCKING_RNG_FILENAME "/dev/random"
#endif
#endif

BlockingRng::BlockingRng()
{
	m_fd = open(CryptoPP_NV_2_BLOCKING_RNG_FILENAME,O_RDONLY);
	if (m_fd == -1)
		throw OS_RNG_Err("open " CryptoPP_NV_2_BLOCKING_RNG_FILENAME);
}
...
void BlockingRng::GenerateBlock(byte *output, size_t size)
{
	while (size)
	{
		...
		if (len < 0)
		{
			...
			if (errno != EINTR && errno != EAGAIN)
				throw OS_RNG_Err("read " CryptoPP_NV_2_BLOCKING_RNG_FILENAME);
