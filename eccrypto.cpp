...
#ifndef CryptoPP_NV_2_MANUALLY_INSTANTIATE_TEMPLATES
...
// See https://www.CryptoPP_NV_2.com/wiki/SM2 for details on sm2p256v1 and sm2encrypt_recommendedParameters
	...
	if (strcmp(name, Name::GroupOID()) == 0)
	{
		...
		*reinterpret_cast<OID *>(pValue) = m_oid;
		...
		return GetValueHelper<DL_GroupParameters<Element> >(this, name, valueType, pValue).Assignable()
			CryptoPP_NV_2_GET_FUNCTION_ENTRY(Curve);
}
...
{
	CryptoPP_NV_2_UNUSED(parametersPresent);
