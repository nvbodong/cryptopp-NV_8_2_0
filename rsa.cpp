...
#if defined(CryptoPP_NV_2_DEBUG) && !defined(CryptoPP_NV_2_DOXYGEN_PROCESSING) && !defined(CryptoPP_NV_2_IS_DLL)
#include "sha3.h"
#include "pssr.h"
...
bool RSAFunction::GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
{
	return GetValueHelper(this, name, valueType, pValue).Assignable()
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(Modulus)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(PublicExponent)
		;
}
...
void RSAFunction::AssignFrom(const NameValuePairs &source)
{
	AssignFromHelper(this, source)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(Modulus)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(PublicExponent)
		;
}
...
void InvertibleRSAFunction::GenerateRandom(RandomNumberGenerator &rng, const NameValuePairs &alg)
{
	...
	CryptoPP_NV_2_ASSERT(modulusSize >= 16);
	if (modulusSize < 16)
		throw InvalidArgument("InvertibleRSAFunction: specified modulus size is too small");

	m_e = alg.GetValueWithDefault(Name::PublicExponent(), Integer(17));

	CryptoPP_NV_2_ASSERT(m_e >= 3); CryptoPP_NV_2_ASSERT(!m_e.IsEven());
	if (m_e < 3 || m_e.IsEven())
	...
	CryptoPP_NV_2_ASSERT(m_d.IsPositive());
...
bool InvertibleRSAFunction::GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
{
	return GetValueHelper<RSAFunction>(this, name, valueType, pValue).Assignable()
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(Prime1)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(Prime2)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(PrivateExponent)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(ModPrime1PrivateExponent)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(ModPrime2PrivateExponent)
		CryptoPP_NV_2_GET_FUNCTION_ENTRY(MultiplicativeInverseOfPrime2ModPrime1)
		;
}
...
void InvertibleRSAFunction::AssignFrom(const NameValuePairs &source)
{
	AssignFromHelper<RSAFunction>(this, source)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(Prime1)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(Prime2)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(PrivateExponent)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(ModPrime1PrivateExponent)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(ModPrime2PrivateExponent)
		CryptoPP_NV_2_SET_FUNCTION_ENTRY(MultiplicativeInverseOfPrime2ModPrime1)
		;
}
