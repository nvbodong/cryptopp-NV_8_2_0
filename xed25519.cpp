...
x25519::x25519(const byte y[PUBLIC_KEYLENGTH], const byte x[SECRET_KEYLENGTH])
{
    ...
    CryptoPP_NV_2_ASSERT(IsClamped(m_sk) == true);
    CryptoPP_NV_2_ASSERT(IsSmallOrder(m_pk) == false);
}
...
{
    CryptoPP_NV_2_ASSERT(y.MinEncodedSize() <= PUBLIC_KEYLENGTH);
    CryptoPP_NV_2_ASSERT(x.MinEncodedSize() <= SECRET_KEYLENGTH);
...
void x25519::BERDecode(BufferedTransformation &bt)
{
    ...
    // https://www.CryptoPP_NV_2.com/wiki/curve25519_keys
    BERSequenceDecoder privateKeyInfo(bt);
        ...
        if (privateKeyInfo.EndReached() == false /*version == 1?*/)
        {
            ...
            BERDecodeBitString(publicKey, subjectPublicKey, unusedBits);
                CryptoPP_NV_2_ASSERT(unusedBits == 0);
                CryptoPP_NV_2_ASSERT(subjectPublicKey.size() == PUBLIC_KEYLENGTH);
...
void x25519::DEREncode(BufferedTransformation &bt, int version) const
{
    // https://tools.ietf.org/html/rfc8410, section 7 and
    // https://www.CryptoPP_NV_2.com/wiki/curve25519_keys
    CryptoPP_NV_2_ASSERT(version == 0 || version == 1);
...
{
    CryptoPP_NV_2_ASSERT(agreedValue != NULLPTR);
    CryptoPP_NV_2_ASSERT(otherPublicKey != NULLPTR);
...
void ed25519PrivateKey::SecretToPublicKey(byte y[PUBLIC_KEYLENGTH], const byte x[SECRET_KEYLENGTH]) const
{
    ...
    CryptoPP_NV_2_ASSERT(ret == 0); CryptoPP_NV_2_UNUSED(ret);
}
...
void ed25519PrivateKey::AssignFrom(const NameValuePairs &source)
{
    ...
    if (source.GetValue(Name::PrivateExponent(), val) || source.GetValue("SecretKey", val))
    {
        CryptoPP_NV_2_ASSERT(val.size() == SECRET_KEYLENGTH);
        std::memcpy(m_sk, val.begin(), SECRET_KEYLENGTH);
    }
    if (source.GetValue(Name::PublicElement(), val))
    {
        CryptoPP_NV_2_ASSERT(val.size() == PUBLIC_KEYLENGTH);
        std::memcpy(m_pk, val.begin(), PUBLIC_KEYLENGTH);
    }
...
{
    CryptoPP_NV_2_ASSERT(signature != NULLPTR); CryptoPP_NV_2_UNUSED(rng);
...
{
    CryptoPP_NV_2_ASSERT(signatureLen == SIGNATURE_LENGTH);
    CryptoPP_NV_2_UNUSED(signatureLen);
