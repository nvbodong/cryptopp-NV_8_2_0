...
#ifndef CryptoPP_NV_2_INLINE
# if defined(CryptoPP_NV_2_DEBUG)
#  define CryptoPP_NV_2_INLINE static
# else
#  define CryptoPP_NV_2_INLINE inline
# endif
#endif

// Uncomment for benchmarking C++ against SSE or NEON.
// Do so in both speck.cpp and speck-simd.cpp.
// #undef CryptoPP_NV_2_SSSE3_AVAILABLE
// #undef CryptoPP_NV_2_SSE41_AVAILABLE
// #undef CryptoPP_NV_2_ARM_NEON_AVAILABLE

ANONYMOUS_NAMESPACE_BEGIN

using CryptoPP_NV_2::word32;
using CryptoPP_NV_2::word64;
using CryptoPP_NV_2::rotlConstant;
using CryptoPP_NV_2::rotrConstant;
...
///   kind of messy.
template <class W>
CryptoPP_NV_2_INLINE void TF83(W& x, W& y, const W k)
{
    x = rotrConstant<8>(x);
    x += y; x ^= k;
    y = rotlConstant<3>(y);
...
///   kind of messy.
template <class W>
CryptoPP_NV_2_INLINE void TR83(W& x, W& y, const W k)
{
    y ^= x;
    y = rotrConstant<3>(y);
    x ^= k; x -= y;
...
CryptoPP_NV_2_INLINE void SPECK_Encrypt(W c[2], const W p[2], const W k[R])
{
    c[0]=p[0]; c[1]=p[1];
...
/// \param k subkey array
template <class W, unsigned int R>
CryptoPP_NV_2_INLINE void SPECK_Decrypt(W p[2], const W c[2], const W k[R])
{
    p[0]=c[0]; p[1]=c[1];
...
template <class W, unsigned int R>
CryptoPP_NV_2_INLINE void SPECK_ExpandKey_2W(W key[R], const W k[2])
{
    CryptoPP_NV_2_ASSERT(R==32);
    W i=0, B=k[0], A=k[1];
...
/// \param k user key array
template <class W, unsigned int R>
CryptoPP_NV_2_INLINE void SPECK_ExpandKey_3W(W key[R], const W k[3])
{
    CryptoPP_NV_2_ASSERT(R==33 || R==26);
...
/// \param k user key array
template <class W, unsigned int R>
CryptoPP_NV_2_INLINE void SPECK_ExpandKey_4W(W key[R], const W k[4])
{
    CryptoPP_NV_2_ASSERT(R==34 || R==27);
...
#if defined(CryptoPP_NV_2_SSE41_AVAILABLE)
...
#if defined(CryptoPP_NV_2_SSSE3_AVAILABLE)
...
#if defined(CryptoPP_NV_2_POWER8_AVAILABLE)
...
{
#if (CryptoPP_NV_2_SPECK64_ADVANCED_PROCESS_BLOCKS)
# if (CryptoPP_NV_2_SSE41_AVAILABLE)
    if (HasSSE41())
        return "SSE4.1";
# endif
# if (CryptoPP_NV_2_ARM_NEON_AVAILABLE)
...
{
    CryptoPP_NV_2_ASSERT(keyLength == 12 || keyLength == 16);
...
{
#if (CryptoPP_NV_2_SPECK128_ADVANCED_PROCESS_BLOCKS)
# if (CryptoPP_NV_2_SSSE3_AVAILABLE)
    if (HasSSSE3())
        return "SSSE3";
# endif
# if (CryptoPP_NV_2_ARM_NEON_AVAILABLE)
...
{
    CryptoPP_NV_2_ASSERT(keyLength == 16 || keyLength == 24 || keyLength == 32);
    CryptoPP_NV_2_UNUSED(params);
...
#if defined(CryptoPP_NV_2_SPECK64_ADVANCED_PROCESS_BLOCKS)
...
#endif  // CryptoPP_NV_2_SPECK64_ADVANCED_PROCESS_BLOCKS

#if defined(CryptoPP_NV_2_SPECK128_ADVANCED_PROCESS_BLOCKS)
...
#endif  // CryptoPP_NV_2_SPECK128_ADVANCED_PROCESS_BLOCKS
