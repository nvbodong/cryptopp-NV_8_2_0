...
#if defined(CryptoPP_NV_2_WIN32_AVAILABLE)
...
double TimerBase::ConvertTo(TimerWord t, Unit unit)
{
	...
	CryptoPP_NV_2_ASSERT(static_cast<unsigned int>(unit) < COUNTOF(unitsPerSecondTable));
...
unsigned long TimerBase::ElapsedTime()
{
	...
	CryptoPP_NV_2_ASSERT(elapsed <= (double)ULONG_MAX);
...
TimerWord Timer::GetCurrentTimerValue()
{
	...
	return now.QuadPart;
#elif defined(CryptoPP_NV_2_UNIX_AVAILABLE)
	timeval now;
...
{
#if defined(CryptoPP_NV_2_WIN32_AVAILABLE) && defined(THREAD_TIMER_AVAILABLE)
...
#elif defined(CryptoPP_NV_2_WIN32_AVAILABLE) && !defined(THREAD_TIMER_AVAILABLE)
	LARGE_INTEGER now;
	if (!QueryPerformanceCounter(&now))
	{
		const DWORD lastError = GetLastError();
