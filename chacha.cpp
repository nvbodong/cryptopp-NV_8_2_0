...
#define CHACHA_OUTPUT(x){\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 0, x0 + state[0]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 1, x1 + state[1]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 2, x2 + state[2]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 3, x3 + state[3]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 4, x4 + state[4]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 5, x5 + state[5]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 6, x6 + state[6]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 7, x7 + state[7]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 8, x8 + state[8]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 9, x9 + state[9]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 10, x10 + state[10]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 11, x11 + state[11]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 12, x12 + state[12]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 13, x13 + state[13]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 14, x14 + state[14]);\
    CryptoPP_NV_2_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 15, x15 + state[15]);}

ANONYMOUS_NAMESPACE_BEGIN

// Hacks... Bring in all symbols, and supply
// the stuff the templates normally provide.
using namespace CryptoPP_NV_2;
...
// Also see https://github.com/weidai11/CryptoPP_NV_2/issues/732.
        ...
        byte *output, const byte *input, size_t iterationCount)
{
    do
    {
        ...
        if (iterationCount)
        {
            ...
            CryptoPP_NV_2_KEYSTREAM_OUTPUT_SWITCH(CHACHA_OUTPUT, BYTES_PER_ITERATION);
...
#endif
#if (CryptoPP_NV_2_ALTIVEC_AVAILABLE)
    if (HasAltivec())
        return 16;
    else
#endif
...
{
    CryptoPP_NV_2_ASSERT(key); CryptoPP_NV_2_ASSERT(length == 16 || length == 32);
    CryptoPP_NV_2_UNUSED(key); CryptoPP_NV_2_UNUSED(length);
...
void ChaCha_Policy::CipherResynchronize(byte *keystreamBuffer, const byte *IV, size_t length)
{
    ...
    CryptoPP_NV_2_ASSERT(length==8); CryptoPP_NV_2_UNUSED(length);
...
{
    CryptoPP_NV_2_ASSERT(key); CryptoPP_NV_2_ASSERT(length == 32);
    CryptoPP_NV_2_UNUSED(length);
...
{
    CryptoPP_NV_2_UNUSED(keystreamBuffer), CryptoPP_NV_2_UNUSED(length);
    CryptoPP_NV_2_ASSERT(length==12);
...
void ChaChaTLS_Policy::SeekToIteration(lword iterationCount)
{
    ...
    CryptoPP_NV_2_ASSERT(iterationCount <= std::numeric_limits<word32>::max());
    m_state[12] = (word32)iterationCount;  // low word
}
        ...
        byte *output, const byte *input, size_t iterationCount)
{
    ...
    // Also see https://github.com/weidai11/CryptoPP_NV_2/issues/790 and
    // https://mailarchive.ietf.org/arch/msg/cfrg/gsOnTJzcbgG6OqD8Sc0GO5aR_tU
    // CryptoPP_NV_2_ASSERT(discard==0);
}
...
void XChaCha20_Policy::SeekToIteration(lword iterationCount)
{
    ...
    CryptoPP_NV_2_ASSERT(0); CryptoPP_NV_2_UNUSED(iterationCount);
}
