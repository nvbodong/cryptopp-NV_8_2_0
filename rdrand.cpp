...
//   at Crypto++ 5.6.3. At compile time, it uses CryptoPP_NV_2_BOOL_{X86|X32|X64}
...
#if defined(CryptoPP_NV_2_CPUID_AVAILABLE) && !defined(CryptoPP_NV_2_DISABLE_ASM)

# if defined(CryptoPP_NV_2_MSC_VERSION)
#   define MASM_RDRAND_ASM_AVAILABLE 1
#   define MASM_RDSEED_ASM_AVAILABLE 1
# endif

# if (__SUNPRO_CC >= 0x5100) || (CryptoPP_NV_2_APPLE_CLANG_VERSION >= 30000) || \
     (CryptoPP_NV_2_CLANG_VERSION >= 20800) || (CryptoPP_NV_2_GCC_VERSION >= 30200)
#   define GCC_RDRAND_ASM_AVAILABLE 1
#   define GCC_RDSEED_ASM_AVAILABLE 1
# endif

#endif  // CryptoPP_NV_2_CPUID_AVAILABLE

typedef unsigned char byte;

#if MASM_RDRAND_ASM_AVAILABLE
extern "C" void CryptoPP_NV_2_FASTCALL MASM_RDRAND_GenerateBlock(byte*, size_t);
#endif

#if MASM_RDSEED_ASM_AVAILABLE
extern "C" void CryptoPP_NV_2_FASTCALL MASM_RDSEED_GenerateBlock(byte*, size_t);
#endif
...
{
    CryptoPP_NV_2_UNUSED(output);  // MSC warning
...
#if (CryptoPP_NV_2_BOOL_X64 || CryptoPP_NV_2_BOOL_X32)
// Fills 8 bytes
...
{
    CryptoPP_NV_2_ASSERT((output && size) || !(output || size));
    if (size == 0) return;
...
#else  // not CryptoPP_NV_2_CPUID_AVAILABLE
