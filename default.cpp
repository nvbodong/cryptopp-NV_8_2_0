...
{
	CryptoPP_NV_2_COMPILE_ASSERT((int)SALTLENGTH <= DIGESTSIZE);
...
{
	CryptoPP_NV_2_COMPILE_ASSERT((int)SALTLENGTH <= (int)DIGESTSIZE);
	CryptoPP_NV_2_COMPILE_ASSERT((int)BLOCKSIZE <= (int)DIGESTSIZE);
}
...
{
	CryptoPP_NV_2_UNUSED(inString); CryptoPP_NV_2_UNUSED(length);
	m_filter->MessageEnd();
}
